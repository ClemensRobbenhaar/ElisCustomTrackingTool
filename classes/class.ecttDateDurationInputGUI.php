...
class ecttDateDurationInputGUI
{
	...
	public function __construct($title, $postvar)
	{
		...
		$this->switch->setOptionTitle($this->txt('ectt_period_filter_label_use'));
	...
	{
		$tpl = ilElisCustomTrackingToolPlugin::_getInstance()->getTemplate('tpl.ectt_date_duration_input.html', true, true);
			
		/*	new ilTemplate(
			'tpl.ectt_date_duration_input.html', true, true, PLUGIN_PATH
		);*/

		$tpl->setVariable('DURATION_SWITCH_LABEL', $this->txt('ectt_filter_period_label'));
		$tpl->setVariable('DURATION_SWITCH_FIELD', $this->switch->getTableFilterHTML());

		$tpl->setVariable('DURATION_FROM_LABEL', $this->txt('ectt_filter_period_start'));
		$tpl->setVariable('DURATION_FROM_FIELD', $this->start->render());

		$tpl->setVariable('DURATION_TO_LABEL', $this->txt('ectt_filter_period_end'));
