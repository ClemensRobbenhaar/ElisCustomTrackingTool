...
class ecttTrackingToolbarGUI
{
	...
	{
		ilElisCustomTrackingToolPlugin::_includeClass('class.ecttDateDurationInputGUI.php');
		$this->tplpath = ilElisCustomTrackingToolPlugin::_getDir();
	...
	{
		$input_label = $this->txt('ectt_filter_'.$filter.'_label');
		...
		switch($filter)
		{
			case self::FILTER_OBJECT:
				...
				$form_options = array(
					self::INPUT_OPTION_ALL_VALUE => $this->txt('ectt_filter_option_all')
				);
	...
	private function renderViewBySwitch($tpl)
	{
		...
		$input = new ilSelectInputGUI(
			$this->txt('ectt_view_by_switch_label'), 'viewby'
		);
		...
		{
			$options[$option] = $this->txt('ectt_viewby_option_'.$option);
		}
	...
	private function renderNumbersSwitch($tpl)
	{
		...
		$input = new ilSelectInputGUI(
			$this->txt('ectt_numbers_switch_label'), 'numbers'
		);
		...
		{
			$options[$option] = $this->txt('ectt_numbers_option_'.$option);
		}
	...
	public function getHTML()
	{
		...
		$label = $this->txt('ectt_toolbar_submit_create_report_screen');
		...
		$tpl->setCurrentBlock('toolbar_commands');
		$label = $this->txt('ectt_toolbar_export');
		$tpl->setVariable('SUBMIT_LABEL', $label);
